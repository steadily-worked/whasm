...
fn 열거형() {
    ...
    let _너비 = 도형::너비;
    let _높이 = 도형::높이;
    // enum을 선언한 후, enum::variant의 형태로 변수를 선언할 수 있다.

    fn route(rectangle: 도형) {}
    route(도형::너비); // 파라미터로 enum의 variant 넣을 수 있다.
    route(도형::높이);

    struct 너비4개(u8, u8, u8, u8);
    struct 높이(String);
...
fn 열거형2() {
    impl Message {
        fn call(&self) {
            // 호출한 열거형의 값을 가져옴. m은 이 메소드 안에서, `self`가 됨.
            match &self {
                // ..
            }
        }
...
fn option열거형() {
    ...
    let _숫자들의_합 = 어떤_숫자.unwrap_or(0) + 어떤_숫자_2; // 에러 발생: Option<i32>와 i32는 다른 타입이므로. Option<i32>는 타입스크립트로 치면 number | undefined 이고, i32는 number라서, 없을 수도 있는 값을 더하려고 하므로 문제가 생긴다.
